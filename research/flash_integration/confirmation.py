...
class BudgetedFlash(Flash):
 current_kind=None
 def call(self,messages,*,tools=None,thinking=False,limit=2048):
  if thinking and self.current_kind=='grounding':limit=32768
  return super().call(messages,tools=tools,thinking=thinking,limit=limit)
...
def run(phase,development=None):
 ...
 hashes={f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in files};preflight=check();cases=[t for t in fixtures() if t['phase']==phase];assert phase=='confirmation'
 ...
 hashes.update({f:hashlib.sha256((ROOT/f).read_bytes()).hexdigest() for f in ['research/flash_integration/confirmation.py','research/flash_integration/CONFIRMATION_AMENDMENT.md']})
 ...
 manifest={'phase':phase,'kind':'live_Flash_native_tool_comparison','source_sha256':hashes,'preflight':preflight,'model_identity_source':'https://api-docs.deepseek.com/news/news260910',
           'max_flash_calls':len(cases)*7,'max_jev_calls':len(cases)*2,'cached_model_labels':False,'grounding_thinking_limit':32768}
 save(directory/'manifest.json',manifest);save(directory/'fixtures.json',cases);dk,jk=credentials();flash=BudgetedFlash(dk);jev=jev_client(jk);calls=[];rows=[]
 try:
