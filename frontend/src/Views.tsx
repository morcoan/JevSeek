...
export function SettingsView({ boot, onSave, onError, onKeys, onTerms }: { boot: Bootstrap; onSave: (preferences: Preferences) => Promise<Preferences>; onError: (message: string) => void; onKeys: () => void; onTerms: () => void }) {
  ...
  return <div className="page-content settings-content">
    ...
    <div className="local-note"><ShieldCheck size={18} /><p><strong>Local, not sandboxed.</strong> Tools have your account’s permissions. Selected task context is sent to DeepSeek, Jev, and enabled MCP services. Session history is saved locally; redaction is best-effort.</p></div>
  </div>;
}
